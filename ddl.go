...
func (c *ColumnDef) buildSQL(dialect Dialect) (string, error) {
	if c.Name == "" {
...
func formatDefaultValue(value interface{}, dialect Dialect) string {
	switch v := value.(type) {
	case Raw:
...
func (c *Constraint) buildSQL(dialect Dialect) (string, error) {
	var parts []string
