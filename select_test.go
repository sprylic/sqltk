...
func TestSelectBuilder_RawWhere(t *testing.T) {
	t.Run("raw where only", func(t *testing.T) {
		q := Select("id").From("users").Where(raw.Cond("age > 18"))
		sql, args, err := q.WithDialect(sqldialect.NoQuoteIdent()).Build()
...
func TestSelectBuilder_GroupBy_Having_OrderBy(t *testing.T) {
	...
	t.Run("group by raw", func(t *testing.T) {
		q := Select("id").From("users").GroupBy(raw.Raw("LEFT(name, 1)"))
		sql, _, err := q.WithDialect(sqldialect.NoQuoteIdent()).Build()
	...
	t.Run("having raw", func(t *testing.T) {
		q := Select("id").From("users").GroupBy("id").Having(raw.Cond("COUNT(*) > 1"))
		sql, args, err := q.WithDialect(sqldialect.NoQuoteIdent()).Build()
...
func TestSelectBuilder_Distinct_Subquery(t *testing.T) {
	...
	t.Run("subquery as column", func(t *testing.T) {
		sub := Select(raw.Raw("COUNT(*)")).From("posts").Where(raw.Raw("posts.user_id = users.id"))
...
func TestSelectBuilder_Alias(t *testing.T) {
	...
	t.Run("alias subquery as column", func(t *testing.T) {
		sub := Select("COUNT(*)").From("orders").Where(raw.Raw("orders.user_id = users.id"))
	...
	t.Run("alias raw in FROM", func(t *testing.T) {
		q := Select("u.id").From(Alias(raw.Raw("users u"), "u_alias"))
		sql, _, err := q.WithDialect(sqldialect.NoQuoteIdent()).Build()
...
func TestSelectBuilder_Compose(t *testing.T) {
	...
	t.Run("compose with subqueries", func(t *testing.T) {
		sub1 := Select("COUNT(*)").From("posts").Where(raw.Raw("user_id = users.id"))
		sub2 := Select("MAX(created_at)").From("posts").Where(raw.Raw("user_id = users.id"))
...
func TestSelectBuilder_Dialect(t *testing.T) {
	t.Run("no quote ident dialect", func(t *testing.T) {
		q := Select("id", "name").From("users").Where(NewCond().Equal("id", 1).
			And(NewCond().Equal("name", "bob"))).WithDialect(sqldialect.NoQuoteIdent())
		sql, _, err := q.WithDialect(sqldialect.NoQuoteIdent()).Build()
	...
	t.Run("mysql dialect", func(t *testing.T) {
		q := Select("id", "name").From("users").Where(NewCond().Equal("id", 1).
			And(NewCond().Equal("name", "bob"))).WithDialect(sqldialect.MySQL())
	...
	t.Run("postgres dialect", func(t *testing.T) {
		q := Select("id", "name").From("users").Where(NewCond().Equal("id", 1).
			And(NewCond().Equal("name", "bob"))).WithDialect(sqldialect.Postgres())
...
func TestSelectBuilder_GetColumns(t *testing.T) {
	...
	t.Run("raw columns", func(t *testing.T) {
		q := Select("id", raw.Raw("COUNT(*)"), raw.Raw("MAX(created_at)")).From("users")
		cols := q.GetColumns()
	...
	t.Run("alias columns", func(t *testing.T) {
		q := Select("id", Alias("name", "user_name"), Alias(raw.Raw("COUNT(*)"), "total")).
			From("users")
		cols := q.GetColumns()
	...
	t.Run("mixed column types", func(t *testing.T) {
		sub := Select("COUNT(*)").From("orders")
		q := Select("id", "name", raw.Raw("MAX(created_at)"), mysqlfunc.Sum("amount"), Alias("email", "user_email"), sub).
			From("users")
		cols := q.GetColumns()
